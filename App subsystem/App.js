  ...
  useEffect(() => {
    ...
    const pollTelemetry = async () => {
      ...
      try {
        ...
        // Use ESP weight event for weight + alarm state
        const eventWeightOn =
          espEvent === "EVENT_WEIGHT_RESTORED"
            ? true
            : espEvent === "EVENT_WEIGHT_REMOVED"
            ? false
            : null;
        // Trigger alarm when ESP says weight was removed
        if (espEvent === "EVENT_WEIGHT_REMOVED") {
          try {
            Notifications.scheduleNotificationAsync({
              content: { title: "Rover Alarm", body: "Item removed from rover." },
              trigger: null,
            });
          } catch (e) {}
        }
